...
yes | rm /data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/dev-small -r

echo '##
## Plug-in for installing small
##

DISTRO_NAME="dev-small https://github.com/stringmanolo/dev-small"
...
' > /data/data/com.termux/files/usr/etc/proot-distro/dev-small.sh

proot-distro remove dev-small;

proot-distro install dev-small;

yes | rm /data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/dev-small -r

cp smallFileSystem /data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/dev-small -r;

# SMALL_COMMAND IS CHECKED INSIDE SHELL TO DECIDE IF TO PRINT MOTD OR NOT 
echo '#!/usr/bin/env bash

if [[ "$1" ]]; then
 touch /data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/dev-small/tmp/.small_command_1;
 proot-distro login dev-small --isolated --fix-low-ports -- /bin/"$@"
 rm /data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs/dev-small/tmp/.small_command_1;
else
  clear;
  proot-distro login dev-small --isolated --fix-low-ports
fi
' > "$HOME"/../usr/bin/dev-small;

chmod +775 "$HOME"/../usr/bin/dev-small;

echo 'Dev System Ready:
